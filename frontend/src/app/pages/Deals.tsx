    ...
    <div className="min-h-screen bg-background">
      <section className="pt-16 pb-10 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="font-display text-3xl lg:text-4xl font-bold text-foreground mb-3 tracking-tight">Current deals</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {DEALS.map(deal => {
            ...
            return (
              <div key={deal.id} className="rounded-lg border border-border p-6 flex flex-col">
                <div className="flex items-center gap-2 mb-4 text-sm text-muted-foreground">
                  <Clock className="w-3.5 h-3.5" />
                  <span>{formatCountdown(remaining)}</span>
                </div>
                <p className="text-3xl font-bold text-foreground mb-1">{deal.percentOff}</p>
                <h3 className="text-lg font-semibold text-foreground mb-2">{deal.title}</h3>
                <p className="text-muted-foreground text-sm mb-5 leading-relaxed flex-1">{deal.description}</p>
                {ended ? (
                  <span
                    aria-disabled="true"
                    className={buttonClassName('secondary', 'md', 'self-start pointer-events-none opacity-60')}
                  >
                    Deal ended
                  </span>
                ) : (
                  <LinkButton to="/products" size="md" className="self-start">
                    Shop this deal
        ...
        <div className="mt-16 bg-surface rounded-lg p-8 border border-border">
          <div className="max-w-md mx-auto text-center">
            <h3 className="text-xl font-semibold text-foreground mb-2">Get deal alerts</h3>
            ...
            >
              <input
                ...
                className="flex-1 px-4 py-2.5 rounded-lg bg-card border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary text-sm transition-colors"
              />
