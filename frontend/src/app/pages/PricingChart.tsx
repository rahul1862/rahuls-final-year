...
export function PricingChart() {
  ...
  return (
    <div className="min-h-screen bg-background">
              ...
              return (
                <div
                  ...
                  className={`rounded-lg border p-6 lg:p-8 flex flex-col ${
                    tier.id === 'plus' ? 'border-primary bg-surface shadow-card-hover' : 'border-border bg-card'
                  }`}
                >
                  {tier.id === 'plus' && (
                    <Badge variant="new" className="self-start mb-3">Most members pick this</Badge>
                  )}
                  <h3 className="text-lg font-semibold text-foreground">{tier.name}</h3>
                  <p className="text-muted-foreground text-sm mt-1 mb-6">{tier.description}</p>

                  <div className="mb-6">
                    <span className="text-3xl font-bold text-foreground">${displayPrice.toFixed(displayPrice % 1 === 0 ? 0 : 2)}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-card rounded-lg border border-border p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="font-display text-xl font-semibold text-foreground">Is it worth it for you?</h2>
                ...
                {ORDER_FREQUENCY_OPTIONS.map(n => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-md text-sm font-medium transition-colors ${
                      ordersPerYear === n ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    {n}/yr
                  </button>
            ...
            {loading ? (
              <div className="h-72 flex items-end gap-6 px-2">
                {tiers.map(tier => (
                  <div key={tier.id} className="flex-1 bg-secondary rounded-t-md animate-pulse" style={{ height: `${25 + Math.random() * 55}%` }} />
                ))}
              </div>
            ) : chartData.length === 0 ? (
              <div className="h-72 flex items-center justify-center">
                <p className="text-muted-foreground text-sm">No plan data to show.</p>
              </div>
            ) : (
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fill: 'var(--muted-foreground)', fontSize: 13 }} axisLine={{ stroke: 'var(--border)' }} tickLine={false} />
                    <YAxis
                      ...
                      label={{ value: 'Dollars per year', angle: -90, position: 'insideLeft', fill: 'var(--muted-foreground)', fontSize: 12 }}
                    />
                    <Tooltip
                    ...
                    />
                    <Legend
                      formatter={(value: string) => <span style={{ color: 'var(--muted-foreground)', fontSize: 13 }}>{value}</span>}
                    />
                    <Bar dataKey="value" name="Estimated yearly value" fill="var(--primary)" radius={[4, 4, 0, 0]} maxBarSize={56} />
                    <Bar dataKey="cost" name="Membership cost" fill="var(--border-strong)" radius={[4, 4, 0, 0]} maxBarSize={56} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
      ...
      <section className="pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-xl font-semibold text-foreground mb-1">Per-order add-ons</h2>
          <p className="text-muted-foreground text-sm mb-6">Not a subscription — added at checkout, order by order, on any plan.</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {addOns.map(addon => (
              <div key={addon.name} className="rounded-lg border border-border p-5 bg-card">
                <h3 className="text-foreground font-medium mb-1.5">{addon.name}</h3>
                <p className="text-muted-foreground text-sm leading-relaxed mb-4">{addon.description}</p>
                <p className="text-foreground font-semibold">
                  ${addon.price} <span className="text-muted-foreground font-normal text-sm">{addon.unit}</span>
                </p>
              </div>
            ))}
          </div>
      ...
      <section className="pb-16">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-xl font-semibold text-foreground mb-6">Questions about membership</h2>
          <div className="space-y-4">
            {faqs.map(item => (
              <div key={item.q} className="border-b border-border pb-4 last:border-0">
                <h3 className="text-foreground text-sm font-medium mb-1.5">{item.q}</h3>
      ...
      <section className="pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-surface rounded-lg border border-border p-8 text-center">
            <h3 className="font-display text-lg font-semibold text-foreground mb-1">Ready to browse?</h3>
            <p className="text-muted-foreground text-sm mb-5">Plans can be changed or cancelled from your account at any time.</p>
