...
export function ComparisonChart() {
  ...
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {platforms.map(platform => (
              ...
              >
                ...
                <h3 className="text-[#0a0a0a] font-semibold mb-1">{platform.name}</h3>
                <p className="text-[#a1a1aa] text-xs mb-4">{platform.description}</p>
                <div className="flex items-baseline gap-1.5 mb-1">
                  <span className="text-[#0a0a0a] font-bold text-lg">{platform.rating}</span>
                  <span className="text-[#a1a1aa] text-xs">/ 5</span>
                </div>
                <p className="text-[#a1a1aa] text-xs">{platform.reviews.toLocaleString()} reviews</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-lg border border-[#e4e4e7] p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="text-xl font-semibold text-[#0a0a0a]">{config.label}</h2>
                ...
                {(Object.keys(metricConfig) as Metric[]).map(m => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-md text-sm font-medium transition-colors ${
                      metric === m ? 'bg-white text-[#0a0a0a] shadow-sm' : 'text-[#71717a] hover:text-[#0a0a0a]'
                    }`}
                  >
            ...
            {loading ? (
              <div className="h-72 flex items-end gap-4 px-2">
                {platforms.map(p => (
                  <div key={p.name} className="flex-1 bg-[#f4f4f5] rounded-t-md animate-pulse" style={{ height: `${30 + Math.random() * 50}%` }} />
                ))}
              </div>
            ) : chartData.length === 0 ? (
              <div className="h-72 flex items-center justify-center">
                <p className="text-[#a1a1aa] text-sm">No comparison data available right now.</p>
              </div>
            ) : (
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                    ...
                    <YAxis
                      ...
                      tickLine={false}
                      width={40}
                      label={{ value: config.axisLabel, angle: -90, position: 'insideLeft', fill: '#a1a1aa', fontSize: 12 }}
                    />
                    <Tooltip
                      cursor={{ fill: '#fafafa' }}
            ...
            <div className="flex items-center gap-5 mt-4 pt-4 border-t border-[#e4e4e7]">
              ...
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-sm bg-[#d4d4d8]" />
                <span className="text-[#71717a] text-xs">Other marketplaces</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-xl font-semibold text-[#0a0a0a] mb-6">Feature by feature</h2>
          <div className="bg-white rounded-lg border border-[#e4e4e7] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#e4e4e7]">
                    <th className="px-6 py-4 text-left font-medium text-[#71717a]">Feature</th>
                    {platforms.map(platform => (
                      <th key={platform.name} className="px-6 py-4 text-center font-medium text-[#0a0a0a]">
                        {platform.name}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-[#e4e4e7]">
                  {featureRows.map(row => (
                    <tr key={row.label}>
                      <td className="px-6 py-4 text-[#0a0a0a]">{row.label}</td>
                      {platforms.map(platform => (
                        <td key={platform.name} className="px-6 py-4 text-center">
                          {row.values[platform.name] ? (
                            <Check className="w-4 h-4 text-[#0a0a0a] mx-auto" />
                          ) : (
                            <X className="w-4 h-4 text-[#d4d4d8] mx-auto" />
                          )}
                        </td>
                      ))}
                    </tr>
      ...
      <section className="pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-[#fafafa] rounded-lg border border-[#e4e4e7] p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
              <h3 className="text-lg font-semibold text-[#0a0a0a] mb-1">Want to see it for yourself?</h3>
